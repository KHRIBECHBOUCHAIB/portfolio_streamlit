...
def projets():
    ...
    with st.expander(
        "Modèle de Prédiction de Performance des Étudiants en Python (Fév 2024 - Mar 2024)"
    ):
        st.markdown(
            """
    ...
    with st.expander(
        "Tableau de Bord de Rentabilité des Techniciens de Maintenance avec Power BI (Mar 2023 - Déc 2023)"
    ):
        st.markdown(
            """
    ...
    with st.expander(
        "Système d'Approbation de Prêts Alimenté par ML (Sep 2023 - Oct 2023)"
    ):
        st.markdown(
            """
    ...
    with st.expander(
        "Système de Gestion de Prospects avec Support Multi-Bases de Données en JAVA ET JAVA EE (Avr 2023 - Mai 2023)"
    ):
        st.markdown(
            """
        **Association :** Agence nationale pour la formation professionnelle des adultes(AFPA).   
    ...
    with st.expander(
        "Site Web de Cinéma Local avec Vue.js et Intégration d'API de Films (Mar 2023 - Avr 2023)"
    ):
        st.markdown(
            """
        **Association :** Agence nationale pour la formation professionnelle des adultes(AFPA).   
    ...
    with st.expander(
        "Système de Base de Données de Gestion de Musée avec SQL (Jan 2023 - Fév 2023)"
    ):
        st.markdown(
            """
        Association : AFPA.
...
def contact():
    ...
    st.markdown(
        "Veuillez entrer vos informations dans les champs ci-dessous pour me contacter."
    )
    ...
    with st.form(key='contact_form'):
        ...
        message = st.text_area(
            "Message", placeholder="Écrivez votre message ici...", height=150
        )
...
selected_page = st.sidebar.selectbox(
    "Aller à", list(options_nav.keys()), format_func=lambda x: options_nav[x]
)
